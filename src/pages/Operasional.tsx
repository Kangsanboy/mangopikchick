...
const Operasional = () => {
  ...
  const loadCategories = async () => {
    // Filter biar kategori gaji lama ga muncul di dropdown kalau masih ada sisa di DB
    const { data } = await supabase.from('expense_categories')
      .select('*')
      .eq('is_active', true)
      .not('name', 'ilike', '%gaji%') // Filter: Jangan tampilkan kategori yang ada kata 'gaji'
      .order('name');
  ...
  const addExpense = async () => {
    ...
    if (error) toast({ title: "Gagal", description: error.message, variant: "destructive" });
      ...
      setCatName(""); setNote(""); setAmount(""); loadExpenses();
    }
    setLoading(false);
  };
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div><h1 className="text-3xl font-bold text-gray-900">Operasional</h1><p className="text-gray-600 mt-1">Catat biaya operasional harian (Non-Gaji)</p></div>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              ...
              <div className="md:col-span-2"><Label>Keterangan</Label><Input placeholder="Contoh: Beli pulsa listrik" value={note} onChange={e => setNote(e.target.value)} /></div>
        ...
        <Card>
          <CardHeader><CardTitle className="flex gap-2 text-gray-800"><Wallet className="h-5 w-5 text-orange-600"/> Riwayat Pengeluaran</CardTitle></CardHeader>
          <CardContent>
            {expenses.length > 0 ? (
